...
class ACPServer:
    ...
    async def load_session(
        ...
        logger.info(
            "Loading session: {id} for working directory: {cwd}", id=session_id, cwd=cwd
        )
        assert self.conn is not None, "ACP client not connected"
        assert self.client_capabilities is not None, "ACP connection not initialized"
        ...
        if session is None:
            logger.error(
                "Session not found: {id} for working directory: {cwd}",
                id=session_id,
                cwd=cwd,
            )
        ...
        return acp.schema.ListSessionsResponse(
            sessions=[
                acp.schema.SessionInfo(
                    ...
                    session_id=s.id,
                    title=s.title,
                    updated_at=datetime.fromtimestamp(s.updated_at)
                    .astimezone()
                    .isoformat(),
                )
                for s in sessions
    ...
    async def set_session_model(
        ...
        if new_model is None:
            logger.error(
                "Model not found: {model_key}", model_key=model_id_conv.model_key
            )
        ...
        assert config.is_from_default_location, (
            "`kimi acp` must use the default config location"
        )
