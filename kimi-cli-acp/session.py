...
_terminal_tool_call_ids = ContextVar[set[str] | None](
    "terminal_tool_call_ids", default=None
)
...
class ACPSession:
            ...
            async for msg in self._cli.run(user_input, self._turn_state.cancel_event):
                match msg:
                    ...
                    case ToolCallRequest():
                        logger.warning(
                            "Unexpected ToolCallRequest in ACP session: {msg}", msg=msg
                        )
    ...
    async def _handle_approval_request(self, request: ApprovalRequest):
        ...
        try:
            ...
            logger.debug(
                "Requesting permission for action: {action}", action=request.action
            )
            ...
            if isinstance(response.outcome, acp.schema.AllowedOutcome):
                ...
                if option_id == "approve":
                    logger.debug(
                        "Permission granted for: {action}", action=request.action
                    )
                    request.resolve("approve")
                elif option_id == "approve_for_session":
                    logger.debug(
                        "Permission granted for session: {action}",
                        action=request.action,
                    )
                    request.resolve("approve_for_session")
                else:
                    logger.debug(
                        "Permission denied for: {action}", action=request.action
                    )
                    request.resolve("reject")
            else:
                # cancelled
                logger.debug(
                    "Permission request cancelled for: {action}", action=request.action
                )
                request.resolve("reject")
        except Exception:
